...
sudo git clone https://github.com/sloopstash/kickstart-ansible.git kickstart-ansible'''
      }
    }
...
'''
    ...
    stage('CI: Bootstrap testing environment') {
      steps {
        sh '''cd kickstart-docker
sudo docker compose -f compose/crm.yml --env-file compose/${qa2}.env -p sloopstash-${qa2}-crm up -d
...
'''
    ...
    stage('CD: Execute deployment') {
      steps {
        ...
        ansiblePlaybook(playbook: 'playbook/crm/app.yml', credentialsId: 'ansible-node-ssh', inventory: 'inventory/stg', limit: 'sloopstash-stg-crm-app-1', tags: 'setup,update,config,stop,start')
        ansiblePlaybook(playbook: 'playbook/nginx.yml', credentialsId: 'ansible-node-ssh', inventory: 'inventory/stg', limit: 'sloopstash-stg-crm-nginx-1', tags: 'setup,update,config,stop,start')
